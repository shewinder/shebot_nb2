...
from hoshino.modules.customreply._handler import (
    fullmatch,
    keyword,
    rex,
    chain,
...
def batch_delete(matcher: str, words: List[str]):
    CustomReply.delete().where(
        CustomReply.matcher == matcher, CustomReply.word.in_(words)
    ).execute()
...
def edit_item(matcher: str, word: str, reply: str):
    CustomReply.update(reply=reply).where(
        CustomReply.matcher == matcher, CustomReply.word == word
    ).execute()
    for handler in chain:
