...
from .data_source import choose_image, get_user_image

from hoshino.sres import Res as R
from .image import draw, gen_imgs_preview
...
async def switch_img(bot: Bot, event: MessageEvent, state: T_State):
    ...
    if pic:
        ...
        await bot.send(event, f"不存在的表情 {name}")
...
async def generate_img(bot: Bot, event: MessageEvent, state: T_State):
    ...
    msg = match.group(1)
    uid = event.user_id
    image_name = get_user_image(uid)
    pic = draw(msg, image_name)
    await bot.send(event, pic)


imgl = sv.on_command(
    "image list", aliases={"选图列表", "imgswl", "IMGSWL"}, only_group=False
)
