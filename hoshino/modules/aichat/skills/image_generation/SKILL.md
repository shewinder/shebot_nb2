...
| novaAnimeXL_ilV180 | comfyui.py | **英文** | 动漫/二次元风格，色彩鲜艳，细节丰富，原生 1024x1024 | 文生图 | 完全无审核 |
...
| qwen_image_edit | comfyui.py | **中文** | 多图编辑，支持人物替换、融合、风格迁移 | 多张编辑 | 完全无审核 |
...
1. **判断场景**：
   ...
   - **图像编辑**（用户明确要求"修改这张图""改图""换风格"等）→ 根据输入图片数量选择对应能力的模型：
     - **1 张图** → 选择支持**单图编辑**的模型（`qwen_image_edit_single` / `bytedance/seedream-v4/edit` / `gemini-3.1-flash-image-preview`）
     - **2 张及以上** → 选择支持**多张编辑**的模型（`qwen_image_edit` / `bytedance/seedream-v4/edit` / `gemini-3.1-flash-image-preview`）
...
**调优方法**：
1. 保留用户原始中文描述（qwen CLIP 中文理解极强）
2. 清晰描述编辑意图，如"把A换成B"、"融合两张图"、"去掉背景"
3. **不要翻译为英文**

**调优示例**：
```
用户: "把图一右边的女孩换成图二的女孩"
AI 调优后: "把图一右边的女孩换成图二的女孩"
```
...
2. 选择支持编辑的模型（qwen_image_edit、gemini-3.1-flash-image-preview 或 bytedance/seedream-v4/edit）
3. 按模型调优策略处理编辑指令
4. 调用对应协议脚本：
