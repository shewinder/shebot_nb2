...
脚本通过对应环境变量读取 API 密钥，无需额外配置。

## 模型选择策略

**默认优先使用本地 ComfyUI（z_image_turbo），不消耗 API 费用。** 仅在以下情况才使用付费模型：
- 用户明确要求使用某个付费模型
- 需要图像编辑（图生图）能力——gemini-3-pro-image-preview 和 bytedance/seedream-v4/edit 均支持
- ComfyUI 不可用或报错

## 可用模型

| 模型 | 协议脚本 | Prompt 语言 | 特点 | 能力 |
|------|---------|------------|------|------|
...
1. **判断场景**：是否涉及图像编辑？→ 是则在 gemini 和 atlas 间按质量需求选择；否则默认 comfyui
...
execute_script(
    ...
    script_path="scripts/gemini.py",
    args=["--prompt", "A cute orange tabby cat sleeping under a cherry blossom tree, soft pink petals falling, warm sunlight filtering through branches, masterpiece, best quality, highly detailed, anime style",
...
execute_script(
    skill_name="image_generation",
    script_path="scripts/atlascloud.py",
    args=["--prompt", "高清细节，精美画质，一只可爱的橘猫在樱花树下睡觉，粉色花瓣飘落，温暖阳光透过树枝，治愈系画风",
          "--aspect-ratio", "1:1"]
)
```
...
execute_script(
    ...
    args=["--prompt", "高清细节，精美画质，一只可爱的橘猫在樱花树下睡觉，粉色花瓣飘落，温暖阳光透过树枝，治愈系画风",
          "--aspect-ratio", "3:4", "--model", "z_image_turbo"]
)
```
...
1. 从 `SKILL_IMAGES` 环境变量或对话上下文中找到图片标识符（如 `<user_image_1>`）
2. 选择支持编辑的模型（gemini-3-pro-image-preview 或 bytedance/seedream-v4/edit）
3. 按模型调优策略处理编辑指令
4. 调用对应协议脚本：
...
execute_script(
    skill_name="image_generation",
    script_path="scripts/gemini.py",
    args=["--prompt", "Convert to cyberpunk style with neon lights and futuristic elements, masterpiece, best quality",
          "--images", "<user_image_1>"]
)
...
execute_script(
    skill_name="image_generation",
    script_path="scripts/atlascloud.py",
    args=["--prompt", "转换成赛博朋克风格，霓虹灯光和未来元素，高清细节，精美画质",
          "--images", "<user_image_1>"]
)
```
...
- `--model` (可选): 模型名称（如 `gemini-3-pro-image-preview`、`dall-e-3`、`z_image_turbo`），脚本用此名称调用 API 和选择工作流
...
- 脚本成功后会输出 JSON：`{"success": true, "identifier": "<ai_image_1>", "path": "..."}`
- AI 在回复中引用标识符：`已生成 <ai_image_1>`
- 简要说明使用的模型和风格特点
- 不要详细解释每一步做了什么
