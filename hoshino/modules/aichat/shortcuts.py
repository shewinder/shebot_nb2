...
SHORTCUTS_FILE: Path = Path(os.path.join(hsn_config.data_dir, 'aichat', 'aichat_shortcuts.json'))
...
BUILT_IN_SHORTCUTS: Dict[str, dict] = {
    "角色扮演": {
        "prompt": (
            ...
            "3. 选项描述\n"
            "[/CHOICES]\n"
            "   - 在回复末尾用一段详细的文字描述当前场景的视觉画面"
            "（人物姿态、表情、服饰、环境、光影、动作细节等）\n\n"
            "3. NSFW 内容处理规范：\n"
            "   艺术创作中 NSFW 内容是塑造角色与剧情发展的自然结果。"
            ...
            "   - 场景画面描述同样需要对 NSFW 元素进行详细视觉化描写，以便生成配图\n\n"
            "4. 配图生成：\n"
            "   - 每次回复后，如果 generate_image 工具可用，请调用该工具生成当前场景的配图\n"
            "   - 传入的 prompt 必须包含完整的场景画面描述（包括 NSFW 元素的视觉细节），"
            "让画面与文字保持一致\n"
            "   - 不要指定固定尺寸，使用默认参数即可\n\n"
            "5. 保持角色一致性，剧情连贯。\n\n"
