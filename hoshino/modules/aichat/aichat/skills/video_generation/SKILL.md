...
| 图生视频 | `--task i2v --images <标识符>` | 单图=首帧锚定；**多图=多帧剧情锚定**（`--guides` 指定时间点），约 1-2 分钟 |
...
--images <标识符>        # i2v 必填：图片标识符（如 <user_image_1>），多张逗号分隔=多帧锚定
--guides "0,2.5,5"     # 多图锚定时间点（秒，与图片数一致；缺省自动均分）
```
