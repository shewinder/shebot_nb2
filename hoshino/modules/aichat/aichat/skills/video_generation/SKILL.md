...
- 分辨率：默认 480p（864×480）；档位 360p/480p/540p/720p/768p/1080p（t2v 固定尺寸；i2v/ref 按图比例自适应到档位像素上限）；24fps MP4（含 AAC 音轨）。720p 以上很慢（768p 约 3-5 倍耗时，1080p 更慢且有 16GB 显存 OOM 风险，谨慎使用）
...
--resolution 360p|480p|540p|720p|768p|1080p  # 分辨率档位（默认 480p；t2v 固定尺寸，i2v/ref 按图比例+档位上限）
...
**`edit` 模式场景数硬性规则（必须先探测再规划，防止成片丢尾段）**：
1. 提交前**必须**先 `scripts/video_tools.py --probe --video <源标识符>`
2. **按公式计算场景数**（禁止按"时长÷5 秒"之类自行估算）：源转 24fps 后帧数 = 时长秒数 × 24（向上取整）；每窗口 124 帧、续段裁 22 帧 Context 后按 102 帧推进 → `场景数 = ceil((帧数 − 124) / 102) + 1`，源不足 124 帧则为 1。例：20.03s → 481 帧 → ceil(357/102)+1 = 4+1 = **5 个场景**
3. 每个场景 `length` 填 124（脚本会按窗口覆盖）；场景数多于窗口数时多余场景会被忽略
4. 场景不足时脚本会**报错拒绝**（提示所需场景数与缺失秒数），收到该错误时按提示补齐场景重试，不要截断源视频
