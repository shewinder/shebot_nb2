...
3. **记录颜色和方向** — 每框文字的颜色(hex)和排布方向(horizontal/vertical)，这是 Step 3 渲染参数的依据
...
相邻的小框如果属于同一句话，用 `bbox_ids` 合并成一个 translation。合并后 `text` 写完整译文。

**合并条件（需同时满足）：**

1. **视觉优先** — Step 2 的画面分析确认这些框在同一对话框/气泡内。跨气泡 → 禁止合并
