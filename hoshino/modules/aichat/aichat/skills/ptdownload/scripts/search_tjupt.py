#!/usr/bin/env python3
"""
北洋园 (TJUPT) 搜索脚本 — HTML 抓取

用法: python search_tjupt.py <关键词> [--json]
"""
import argparse
import asyncio
import json
...
async def search(keyword: str) -> dict:
    """搜索 北洋园"""
    cookie = os.environ.get("PT_TJUPT_COOKIE", "")
    if not cookie:
        return {"success": False, "error": "北洋园 Cookie 未配置"}
    ...
    rows = soup.select("table.torrents tr.sticky_bg, table.torrents tr.normal_bg")
    results = []
    for row in rows:
        try:
            title_el = row.select_one("table.torrentname a[href*='details.php?id=']")
            if not title_el:
                continue
            title = title_el.get_text(strip=True)
            dl_el = row.select_one("a[href*='download.php?id=']")
            download = urljoin(SEARCH_URL, dl_el["href"]) if dl_el else ""
            size_el = row.select_one("td.rowfollow:nth-of-type(5)")
            size = size_el.get_text(strip=True) if size_el else "-"
            seeders_el = row.select_one("td.rowfollow:nth-of-type(6)")
...
def format_results(data: dict) -> str:
    ...
    if not results:
        return f"🔍 北洋园未找到「{data.get('keyword')}」相关资源"

    lines = [f"🔍 北洋园搜索到 {len(results)} 个资源：\n"]
...
async def download_torrent(url: str) -> Optional[bytes]:
    ...
    cookie = os.environ.get("PT_TJUPT_COOKIE", "")
    if not cookie:
        print("⚠ PT_TJUPT_COOKIE 未配置", file=sys.stderr)
        return None
...
def main():
    parser = argparse.ArgumentParser(description="北洋园搜索与下载")
