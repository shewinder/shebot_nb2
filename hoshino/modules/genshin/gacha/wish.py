...
from .wish_user import WishUser
...
def gacha_once(user: WishUser, gacha_type: int):
    rand = random_int()
    fn = character_probability if gacha_type == GACHA_TYPE.activity.value else weapon_probability
    index_5 = fn(5, user.count_5)
    index_4 = fn(4, user.count_4)
    if rand <= index_5:
        rank = 5
    elif rand <= index_4:
        rank = 4
...
class Wish:
    ...
    def __init__(self, uid, gacha_type, gacha_pool):
        self.user = WishUser(uid, gacha_type)
    ...
    def once(self):
        # 判定星级
        rank = self.choose_rank()
        # 是否为up
        is_up = self.choose_up(rank)

        if rank == 5:
            new_rec = GachaRecord(uid=self.user.uid, gacha_type=self.gacha_type, rank=rank, is_up=is_up)
        elif rank == 4:
            pass
        elif rank == 3:
            pass
