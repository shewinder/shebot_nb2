...
async def _(bot: Bot, event: Event, state: T_State):
    ...
    video = await bv_checker.get_data(url)
    if not video:
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    gid = event.group_id
    try:
        bv_checker.add_group(gid, 'BiliVideoChecker', state['url'], remark=f'{state["upname"]}投稿')
...
async def _(bot: Bot, event: PrivateMessageEvent, state: T_State):
    v: Video = await bv_checker.get_data(state['url'])
    uid = event.user_id
    try:
        bv_checker.add_user(uid, 'BiliVideoChecker', state['url'], remark=f'{state["upname"]}投稿')
        await add_video.finish(f'成功订阅{state["upname"]}的投稿')
    except ValueError as e:
        await add_video.finish(str(e))
...
@del_video.handle()
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    subs = bv_checker.get_group_subs(event.group_id)
...
@del_video.handle()
async def _(bot: Bot, event: PrivateMessageEvent, state: T_State):
    subs = bv_checker.get_user_subs(event.user_id)
    reply = [f'{i}. {sub.remark}' for i,sub in enumerate(subs)]
    state['subs'] = subs
    await bot.send(event, '请发送对应序号选择取消的订阅\n' + '\n'.join(reply))
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    bv_checker.delete_group_sub(event.group_id, sub)
