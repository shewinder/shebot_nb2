...
async def _(bot: Bot, event: Event, state: T_State):
    ...
    dynamic = await bd_checker.get_data(url)
    if not dynamic:
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    gid = event.group_id
    try:
        bd_checker.add_group(gid, 'BiliDynamicChecker', state['url'], remark=f'{state["upname"]}动态')
...
async def _(bot: Bot, event: PrivateMessageEvent, state: T_State):
    v: Dynamic = await bd_checker.get_data(state['url'])
    uid = event.user_id
    try:
        bd_checker.add_user(uid, 'BiliDynamicChecker', state['url'], remark=f'{state["upname"]}动态')
        await add_dynamic.finish(f'成功订阅{state["upname"]}的动态')
    except ValueError as e:
        await add_dynamic.finish(str(e))
...
@del_dynamic.handle()
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    subs = bd_checker.get_group_subs(event.group_id)
...
@del_dynamic.handle()
async def _(bot: Bot, event: PrivateMessageEvent, state: T_State):
    subs = bd_checker.get_user_subs(event.user_id)
    reply = [f'{i}. {sub.remark}' for i,sub in enumerate(subs)]
    state['subs'] = subs
    await bot.send(event, '请发送对应序号选择取消的订阅\n' + '\n'.join(reply))
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    bd_checker.delete_group_sub(event.group_id, sub)
...
async def _(bot: Bot, event: PrivateMessageEvent, state: T_State):
    try:
        ...
        await del_dynamic.finish('输入有误')
    bd_checker.delete_user_sub(event.user_id, sub)
